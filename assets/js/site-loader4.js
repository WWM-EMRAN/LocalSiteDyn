...
function renderIndexExperienceItem(role) {
  ...
  const descriptionHtml = role.description_list && role.description_list.length > 0 ? `
    <div class="mb-2">
        ...
        <ul class="ms-0">
            ${role.description_list.map(desc => `<li>${desc}</li>`).join('')}
        </ul>
    </div>` : '';
  ...
  const responsibilitiesHtml = `
    <div class="ms-0">
        ...
        <ul class="ms-2">
            ${role.responsibilities_list.map(resp => `<li>${resp}</li>`).join('')}
        </ul>
    </div>`;

  const courseHtml = role.course_involvement && role.course_involvement.length > 0 ? `
    <div class="mb-2">
        <p class="mb-1"><strong>Course Involvement:</strong></p>
        <ul class="ms-2">
            ${role.course_involvement.map(course => `<li><small>${course}</small></li>`).join('')}
        </ul>
    </div>` : '';
  ...
  return `
    <div class="resume-item" id="${role.role_id}" data-aos="fade-up">
        ...
        <p class="mb-2"><strong>About Job:</strong> ${role.about_job}</p>
        <details class="details-box">
            <summary class="badge badge-type border-0 text-white" style="font-size: 11px; cursor: pointer;">
                <i class="bi bi-eye me-1"></i> More Details
            </summary>
        ...
        <div class="resume-item pb-3" id="${item.id_ref}" data-aos="fade-up">
            ...
            <div class="d-flex flex-wrap gap-2 mb-2 align-items-center">
                <button class="badge border-0 badge-dates citation-btn" 
                        onclick="copyToClipboard('${item.citation_text.replace(/'/g, "\\'")}')">
...
function renderIndexContactItem(contact, index) {
    ...
    const target = contact.title === "Email" ? "" : 'target="_blank" rel="noreferrer"';
    ...
    return `
        <div class="col-lg-4 col-md-6 service-item d-flex" data-aos="fade-up" data-aos-delay="${delay}">
            ...
            <div>
                <h4 class="title">
                    <a href="${contact.link}" ${target} class="stretched-link">${contact.title}</a>
                </h4>
            ...
            listWrapper.innerHTML = groupData.items.map((item, index) => {
                ...
                return `
                    <div class="${isLast ? 'mb-0' : 'mb-3'}" id="${item.id_ref}">
                        ...
                        <div class="mt-1 text-justify">
                            ${item.citation_text}
                            ${link ? `<br><span class="text-primary small italic">DOI/Link: ${link}</span>` : ''}
                        </div>
                    </div>
                `;
            }).join('');
...
function renderStandardCVContacts() {
    ...
    gridContainer.innerHTML = `
        ...
        <div class="col-4">
            ...
            <div class="small"><strong>Scholar:</strong> <a href="${contacts.google_scholar.link}" target="_blank">${contacts.google_scholar.text}</a></div>
        </div>
        <div class="col-4">
